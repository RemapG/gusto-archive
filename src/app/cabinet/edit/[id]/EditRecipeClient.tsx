...
export default function EditRecipeClient({ recipe }: EditRecipeClientProps) {
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(
    recipe.category ? recipe.category.split(', ') : ["Основные блюда"]
  );
  ...
  const [steps, setRecipeSteps] = useState<any[]>(
    recipe.steps.length > 0
      ? recipe.steps.map((s) => {
          ...
          const text = isStr ? s : (s?.text || "");
          const imageUrl = isStr ? null : (s?.image_url || null);
          const timer = isStr ? null : (s?.timer || null);
          const videoUrlVal = isStr ? null : (s?.video_url || null);
          return {
            text: text,
            image: null,
            imagePreview: imageUrl,
            existingImageUrl: imageUrl,
            timerMinutes: timer ? (timer / 60).toString() : "",
    ...
    return (
      <div className="min-h-screen bg-[#f6f5f0] flex flex-col items-center justify-center font-serif italic text-2xl gap-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          ...
          <p className="text-xs font-sans uppercase tracking-[0.2em] text-[#8a8883] mt-2 non-italic">
            Перенаправляем к рецепту...
          </p>
        </motion.div>
      </div>
    );
    ...
    <main className="min-h-screen bg-[#f6f5f0] text-[#2d2c2a] selection:bg-[#e8e6df] font-sans pb-32">
      <header className="px-8 py-6 md:px-16 md:py-8 w-full max-w-[1000px] mx-auto">
        <Link
          ...
          className="inline-flex items-center text-[10px] uppercase tracking-widest text-[#8a8883] hover:text-[#2d2c2a] transition-colors font-medium"
        >
      ...
      <div className="max-w-[800px] mx-auto px-8">
        <h1 className="font-serif italic text-4xl mb-2">Редактирование рецепта</h1>
        <p className="text-[10px] uppercase tracking-[0.2em] text-[#8a8883] font-medium mb-12">
          ШАГ {step} ИЗ 3
        </p>

        {error && (
          <div className="mb-8 p-4 bg-red-50 text-red-600 text-sm font-light rounded-sm border border-red-100">
            {error}
          </div>
        )}
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border-b border-[#e2e0d8] py-3 bg-transparent focus:outline-none focus:border-black font-serif italic text-2xl"
                      placeholder="Например: Идеальный Гребешок"
                    />
                  </div>

                  <div className="space-y-6">
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-[#8a8883] mb-3 font-medium">
                        Вид блюда (можно выбрать несколько)
                      </label>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border-b border-[#e2e0d8] py-3 bg-transparent focus:outline-none focus:border-black font-light"
                        placeholder="1000"
                      />
                    </div>
                  </div>
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="space-y-8">
                  <div>
                    <label className="block text-[10px] uppercase tracking-widest text-[#8a8883] mb-2 font-medium">
                      Краткое описание (для витрины)
                    </label>
                    <textarea
                      value={description}
                      onChange={(e) => setDescription(e.target.value)}
                      rows={3}
                      className="w-full border border-[#e2e0d8] rounded-2xl p-4 bg-transparent focus:outline-none focus:border-black font-light resize-none"
                      placeholder="Расскажите о блюде..."
                    />
                  </div>

                  <div>
                    <label className="block text-[10px] uppercase tracking-widest text-[#8a8883] mb-4 font-medium">
                      Список ингредиентов
                    </label>
                    <div className="space-y-3">
                      {ingredients.map((ing, i) => (
                        <div key={i} className="flex gap-3">
                          <input
                            ...
                            className="flex-1 border-b border-[#e2e0d8] py-2 bg-transparent focus:outline-none focus:border-black font-light"
                            placeholder="Например: Морская соль - 10 г"
                          />
                          <button
                            onClick={() => setIngredients(ingredients.filter((_, idx) => idx !== i))}
                            className="text-[#8a8883] hover:text-red-500 transition-colors mt-2"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ...
                      <button
                        ...
                        className="text-[10px] font-medium uppercase tracking-widest flex items-center gap-2 mt-4 hover:opacity-70"
                      >
                ...
                <div className="mt-12 flex justify-between">
                  <button
                    onClick={() => setStep(1)}
                    className="px-6 py-3 rounded-full text-[10px] font-medium uppercase tracking-widest border border-[#e2e0d8] hover:bg-[#f6f5f0] transition-colors flex items-center gap-2"
                  >
                    <ArrowLeft size={14} /> Назад
                  </button>
                  <button
                    onClick={() => setStep(3)}
                    disabled={!description || ingredients.length === 0}
                    className="bg-[#2d2c2a] text-white px-8 py-3 rounded-full text-[10px] font-medium uppercase tracking-widest hover:bg-black transition-colors flex items-center gap-2 disabled:opacity-50"
                  >
                    Далее <ArrowRight size={14} />
                  </button>
              ...
              >
                ...
                <div className="space-y-12">
                  {steps.map((s, i) => (
                    <div key={i} className="relative pl-8 border-l border-[#e2e0d8]">
                      <span className="absolute -left-3 top-0 w-6 h-6 rounded-full bg-[#f6f5f0] border border-[#e2e0d8] flex items-center justify-center text-[10px] font-medium">
                        {i + 1}
                      </span>
                      ...
                      {steps.length > 1 && (
                        <button
                          onClick={() => setRecipeSteps(steps.filter((_, idx) => idx !== i))}
                          className="absolute -left-[4.5rem] top-8 text-[#8a8883] hover:text-red-500 transition-colors p-2"
                        >
                          <Trash2 size={16} />
                        </button>
                      )}
                  ...
                  <button
                    onClick={() =>
                      setRecipeSteps([
                        ...steps,
                        { text: "", image: null, imagePreview: null, timerMinutes: "", existingImageUrl: null, videoUrl: "" }
                      ])
                    }
                    className="text-[10px] font-medium uppercase tracking-widest flex items-center gap-2 hover:opacity-70 bg-[#f6f5f0] px-6 py-3 rounded-full"
                  >
                ...
                <div className="mt-16 pt-8 border-t border-[#e2e0d8] flex justify-between items-center">
                  <button
                    onClick={() => setStep(2)}
                    disabled={saving}
                    className="px-6 py-3 rounded-full text-[10px] font-medium uppercase tracking-widest border border-[#e2e0d8] hover:bg-[#f6f5f0] transition-colors flex items-center gap-2 disabled:opacity-50"
                  >
                    <ArrowLeft size={14} /> Назад
                  </button>
                  <button
                    onClick={handleSave}
                    disabled={saving || !steps[0].text}
                    className="bg-green-700 text-white px-8 py-4 rounded-full text-[10px] font-medium uppercase tracking-widest hover:bg-green-800 transition-colors flex items-center gap-2 disabled:opacity-50"
                  >
                    {saving ? "Сохранение..." : <><Check size={16} /> Сохранить изменения</>}
                  </button>
                </div>
              </motion.div>
            )}
